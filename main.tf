...
resource "libvirt_cloudinit_disk" "this" {
  name           = "commoninit.iso"
  user_data      = var.cloud_init_user_data != "" ? local.cloud_init_user_data : data.template_file.user_data.rendered
  network_config = var.cloud_init_network_config != "" ? local.cloud_init_network_config : data.template_file.network_config.rendered
  pool           = libvirt_pool.this.name
}
